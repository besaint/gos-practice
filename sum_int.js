...
jQuery(function () {
    let RESULTS = {}
    function c_sum_int(array, n) {
        let pointer = Module.__malloc(array.length * 4);
        let offset = pointer / 4;
        Module.HEAP32.set(array, offset);
        let result = Module._sum_int(pointer, n);
        Module.__free(pointer);
        return result;
    }
    function tgo_sum_int(array, n) {
        let { wsTinySumInt, memory, wsTinyAllocateInt } = window.wasm.exports

        let myArrayPtr = wsTinyAllocateInt(array.length);
        let myArray = new Int32Array(memory.buffer, myArrayPtr, array.length);
        myArray.set(array);
        return wsTinySumInt(myArrayPtr, n)
    }
    function go_sum_int(array, n) {
        return wsSumInt(array.join(','), 3)
    }
    $('#sum_int_run').on('click', function () {
        let functions = {
            rust: wasm.sum_int,
            c: c_sum_int,
            tinygo: tgo_sum_int, //window.wasm.exports.wsTinySumInt,
            go: go_sum_int, //window.wsSumInt,
            js: JS.SumInt
        }
