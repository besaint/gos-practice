...
jQuery(function () {
    let RESULTS = {}
    function c_sum_double(array, n) {
        let pointer = Module.__malloc(array.length * 8);
        let offset = pointer / 8;
        Module.HEAPF64.set(array, offset);
        let result = Module._sum_double(pointer, n);
        Module.__free(pointer);
        return result;
    }
    function tgo_sum_double(array, n) {
        let { wsTinySumInt, memory, wsTinyAllocateDouble } = window.wasm.exports

        let myArrayPtr = wsTinyAllocateDouble(array.length);
        let myArray = new Int32Array(memory.buffer, myArrayPtr, array.length);
        myArray.set(array);
        return wsTinySumInt(myArrayPtr, n)
    }
    function go_sum_double(array, n) {
        return wsSumDouble(array.join(','), 3)
    }
    $('#sum_double_run').on('click', function () {
        let functions = {
            rust: wasm.sum_double,
            c: c_sum_double,
            tinygo: tgo_sum_double, //window.wasm.exports.wsTinySumDouble,
            go: go_sum_double, //window.wsSumDouble,
            js: JS.SumDouble
        }
    ...
    function showResults() {
        ...
        Object.entries(RESULTS).forEach(item => {
            ...
            items.push({ lang, timeavg, result, timetotal})
        })
    ...
    function start(test) {
        ...
        setTimeout(() => executeTest(test, 0, [array, array.length]))
    }
})