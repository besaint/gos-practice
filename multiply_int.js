...
jQuery(function () {
    let RESULTS = {}
    $('#multiply_int_run').on('click', function () {
        let functions = {
            rust: wasm.multiply_int,
            c: Module._multiply_int,
            tinygo: window.wasm.exports.wsTinyMultiplyInt,
            go: window.wsMultiplyInt,
            js: JS.MultiplyInt
        }
    ...
    function start(test) {
        ...
        const a = Math.floor(parseFloat($('#first').val()))
        const b = Math.floor(parseFloat($('#second').val()))
